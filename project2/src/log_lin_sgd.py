...
class logistic_reg:
    ...
    def log_acc(self, X_test, y_test):
        prediction = self.log_prediction(X_test)
        n_samples = np.size(y_test, axis=0)
        n_correct = 0.0
    ...
    def plot_accuracy_eta(self, X_train, y_train, X_test, y_test):
        print(int(np.floor(np.size(y_train, axis=0)/4)))
        n_t0 = 50
        n_t1 = 3
        eta0 = np.linspace(0.01, 2, n_t0)
        eta1 = np.array((10,50,10000))
        eta1[-1] = pow(10,5)

        acc_eta = np.zeros((n_t1,n_t0))
        t_eta = np.zeros((n_t1,n_t0))

        for i in range(n_t1):
            for j in range(n_t0):
                t_0 = time()
                self.log_solver(X_train, y_train, minibatch_size=int(np.floor(np.size(y_train, axis=0)/4)), n_epochs = 50, max_iter = 50, t0 = eta0[j], t1 = eta1[i])
                acc_eta[i,j] = self.log_acc(X_test, y_test)
                t_eta[i,j] = time() - t_0
                print(f"Runtime: {t_eta[i,j]:.2f} sec w/ t0 = {eta0[j]:.2f}, t1 = {eta1[i]:.2f}")

                print(f"Accuracy Score: {acc_eta[i,j]}")
        ...
        for i in range(n_t1):
            ax1.plot(eta0, acc_eta[i,:], label=f"SGD, t1 = {eta1[i]}")
        #ax1.tick_params(axis='y', labelcolor=color)
        ax1.set_ylim(0, 1.1)
        ax1.legend(loc=0)

        plt.title("Accuracy Score - Learning Rate")
...
class regression:
    ...
    def cross_validation_sgd(self, X, y, splits):
        ...
        MSE_kfold_ridge = np.zeros((lmb_count,splits))
        MSE_kfold_ridge_sgd = np.zeros((lmb_count,splits))
        #MSE_kfold_lasso = np.zeros((lmb_count,splits))
        MSE_kfold_ols = np.zeros((lmb_count,splits))
        MSE_kfold_ols_sgd = np.zeros((lmb_count,splits))

        R2_kfold_ridge = np.zeros((lmb_count,splits))
        R2_kfold_ridge_sgd = np.zeros((lmb_count,splits))
        #R2_kfold_lasso = np.zeros((lmb_count,splits))
        R2_kfold_ols = np.zeros((lmb_count,splits))
        R2_kfold_ols_sgd = np.zeros((lmb_count,splits))
        ...
        for i in range(lmb_count):
            for j in range(splits):
                ...
                if i == 0:
                    self.set_lmb(0)
                    beta_kfold_ols_sgd = self.lin_solver(X_train_kfold, y_train_kfold, minibatch_size=minibatch_size)
                    MSE_kfold_ols_sgd[i,j] = self.MSE_(X_test_kfold, y_test_kfold, beta_kfold_ols_sgd)
                    R2_kfold_ols_sgd[i,j] = self.R2_(X_test_kfold, y_test_kfold, beta_kfold_ols_sgd)
                ...
                beta_kfold_ridge_sgd = self.lin_solver(X_train_kfold, y_train_kfold, minibatch_size=minibatch_size)
                MSE_kfold_ridge_sgd[i,j] = self.MSE_(X_test_kfold, y_test_kfold, beta_kfold_ridge_sgd)
                R2_kfold_ridge_sgd[i,j] = self.R2_(X_test_kfold, y_test_kfold, beta_kfold_ridge_sgd)
        ...
        MSE_kfold_ols = np.mean(MSE_kfold_ols, axis=1)
        MSE_kfold_ols_sgd = np.mean(MSE_kfold_ols_sgd, axis=1)
        #MSE_kfold_ols[:] = MSE_kfold_ols[0]
        MSE_kfold_ridge = np.mean(MSE_kfold_ridge, axis=1)
        MSE_kfold_ridge_sgd = np.mean(MSE_kfold_ridge_sgd, axis=1)
        #MSE_kfold_lasso = np.mean(MSE_kfold_lasso, axis=1)

        R2_kfold_ols = np.mean(R2_kfold_ols, axis=1)
        R2_kfold_ols_sgd = np.mean(R2_kfold_ols_sgd, axis=1)
        #R2_kfold_ols[:] = R2_kfold_ols[0]
        R2_kfold_ridge = np.mean(R2_kfold_ridge, axis=1)
        R2_kfold_ridge_sgd = np.mean(R2_kfold_ridge_sgd, axis=1)
        #R2_kfold_lasso = np.mean(R2_kfold_lasso, axis=1)


        fig, ax = plt.subplots()
        ax.axhline(y=MSE_kfold_ols[0], color='b', linestyle='--', label="OLS analytic")
        ax.axhline(y=MSE_kfold_ols_sgd[0], color='b', label="OLS SGD")
        ax.plot(lmb, MSE_kfold_ridge_sgd, color='r', label = "Ridge SGD")
        ax.plot(lmb, MSE_kfold_ridge, '--', color='r', label = "Ridge Analytic")
        #ax.plot(lmb, MSE_kfold_lasso, label = "Lasso Regression")
        ...
        fig2, ax2 = plt.subplots()
        ax2.axhline(y=R2_kfold_ols[0], color='b', linestyle='--', label="OLS analytic")
        ax2.axhline(y=R2_kfold_ols_sgd[0], color='b', label="OLS SGD")
        ax2.plot(lmb, R2_kfold_ridge_sgd, color='r', label = "Ridge SGD")
        ax2.plot(lmb, R2_kfold_ridge, '--', color='r',  label = "Ridge Analytic")
        #ax2.plot(lmb, R2_kfold_lasso, label = "Lasso Regression")
