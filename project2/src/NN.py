...
class NeuralNetwork:
    def __init__(self, X, Y, features, hidden_layers, categories):
    ...
    def grid_search(self, X_test, Y_test, Y_train):
        #Perform a grid search to find optimal hyper parameters.
    ...
    #dataclasses are not reset each time, so results become wrong
        for i, eta in enumerate(eta_vals):
            for j, lmbda in enumerate(lmbda_vals):
                NN = NeuralNetwork(self.X, self.Y, self.hidden.features, self.hidden.n, self.output.categories)
